...
TABLE = 'dwh.mysis_tab_proveedores'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

INSERT_COLS = [
    'proveedor_id', 'rso', 'oa', 'dt_in', 'usr_in', 'direccion', 'ciudad', 'pais', 
    'contacto', 'mail', 'fono', 'rut', 'tcosto', 'servicio', 'distribucion_id', 'adias', 
    'ctabanco', 'banco', 'mailpago', 'valevista'
]

INT_COLS_NOTNULL     = {'adias', 'distribucion_id', 'proveedor_id', 'servicio', 'tcosto', 'valevista'}
INT_COLS_NULLABLE    = {'oa', 'usr_in'}
DECIMAL_COLS_NOTNULL = set()
DECIMAL_COLS_NULLABLE= set()
FLOAT_COLS_NOTNULL   = set()
STRING_COLS_NOTNULL  = set()
STRING_COLS_NULLABLE = {'banco', 'ciudad', 'contacto', 'ctabanco', 'direccion', 'fono', 'mail', 'mailpago', 'pais', 'rso', 'rut'}
...
def _prepare_for_insert(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[INSERT_COLS]
    df = df.dropna(subset=['proveedor_id'])

    # Int32 NOT NULL
    _int_nn_defaults = {'adias': 0, 'distribucion_id': 0, 'servicio': 0, 'tcosto': 0, 'valevista': 0}
...
def _fetch_existing_ids(client, ids: list) -> Set[int]:
    ...
    for i in range(0, len(ids), step):
        ...
        q = f"SELECT proveedor_id FROM dwh.mysis_tab_proveedores WHERE proveedor_id IN ({ids_txt})"
        res = client.query(q)
...
def export_data_to_clickhouse(df: pd.DataFrame, *args, **kwargs):
    ...
    for i in range(0, total, chunk_size):
        ...
        # Anti-join por proveedor_id (PK)
        ids = chunk['proveedor_id'].dropna().astype(int).tolist()
        existing = _fetch_existing_ids(client, ids)

        if existing:
            keep_mask = [int(x) not in existing for x in chunk['proveedor_id']]
