...
TABLE = 'dwh.mysis_tab_bodegas'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

INSERT_COLS = [
    'bodega_id', 'bodega_desc', 'oa', 'direccion', 'comuna', 'vta_directa', 'cuota',
    'siglas', 'sii', 'local', 'shopify'
]

INT_COLS_NOTNULL     = {'bodega_id', 'oa', 'vta_directa'}
INT_COLS_NULLABLE    = set()
DECIMAL_COLS_NOTNULL = {'cuota'}
DECIMAL_COLS_NULLABLE= set()
FLOAT_COLS_NOTNULL   = set()
STRING_COLS_NOTNULL  = {'bodega_desc'}
STRING_COLS_NULLABLE = {'comuna', 'direccion', 'local', 'shopify', 'siglas', 'sii'}

# Escala declarada en ClickHouse (system.columns): cuota es Decimal(18, 2).
DECIMAL_QUANT = {'cuota': Decimal('0.01')}
...
def _prepare_for_insert(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[INSERT_COLS]
    df = df.dropna(subset=['bodega_id'])

    # Int32 NOT NULL
    _int_nn_defaults = {'oa': 0, 'vta_directa': 0}
    ...
    # Decimal NOT NULL: se conservan como decimal.Decimal (nunca float64, que es
    # inexacto y hace perder el centavo). La cuantizacion ocurre en _to_python_native.
    _dec_nn_defaults = {'cuota': Decimal('0')}
    for c in DECIMAL_COLS_NOTNULL.intersection(df.columns):
        _dflt = _dec_nn_defaults.get(c, Decimal('0'))
        df[c] = df[c].astype('object').where(pd.notna(df[c]), _dflt)

    # String NOT NULL
    _str_nn_defaults = {'bodega_desc': ''}
...
def _fetch_existing_ids(client, ids: list) -> Set[int]:
    ...
    for i in range(0, len(ids), step):
        ...
        q = f"SELECT bodega_id FROM dwh.mysis_tab_bodegas WHERE bodega_id IN ({ids_txt})"
        res = client.query(q)
...
def export_data_to_clickhouse(df: pd.DataFrame, *args, **kwargs):
    ...
    for i in range(0, total, chunk_size):
        ...
        # Anti-join por bodega_id (PK)
        ids = chunk['bodega_id'].dropna().astype(int).tolist()
        existing = _fetch_existing_ids(client, ids)

        if existing:
            keep_mask = [int(x) not in existing for x in chunk['bodega_id']]
