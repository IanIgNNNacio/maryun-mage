...
TABLE = 'dwh.mysis_tab_marcas'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

INSERT_COLS = [
    'marca_id', 'marca_descripcion', 'oa', 'user_in', 'dt_in'
]

INT_COLS_NOTNULL     = {'marca_id'}
...
STRING_COLS_NULLABLE = {'marca_descripcion'}
...
def _fetch_existing_ids(client, ids: list) -> Set[int]:
    ...
    for i in range(0, len(ids), step):
        ...
        q = f"SELECT marca_id FROM dwh.mysis_tab_marcas WHERE marca_id IN ({ids_txt})"
        res = client.query(q)
...
def export_data_to_clickhouse(df: pd.DataFrame, *args, **kwargs):
    ...
    for i in range(0, total, chunk_size):
        ...
        # Anti-join por marca_id (PK)
        ids = chunk['marca_id'].dropna().astype(int).tolist()
        existing = _fetch_existing_ids(client, ids)

        if existing:
            keep_mask = [int(x) not in existing for x in chunk['marca_id']]
