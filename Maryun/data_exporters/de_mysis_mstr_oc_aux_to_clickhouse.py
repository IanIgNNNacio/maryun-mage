...
TABLE = 'dwh.mysis_mstr_oc_aux'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

INSERT_COLS = [
    'posicion', 'oc_id', 'sku', 'qty', 'mda', 'pu',
    'usr_in', 'dt_in', 'resto', 'descuento', 'ppv'
]

INT_COLS_NOTNULL  = {'posicion'}
INT_COLS_NULLABLE = {'oc_id', 'qty', 'mda', 'usr_in', 'resto'}
DECIMAL_COLS_NOTNULL = {'ppv'}
DECIMAL_COLS_NULLABLE = {'pu', 'descuento'}
DECIMAL_COLS = DECIMAL_COLS_NOTNULL | DECIMAL_COLS_NULLABLE
...
def _prepare_for_insert(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in DECIMAL_COLS_NOTNULL.intersection(df.columns):
        df[c] = pd.to_numeric(df[c], errors='coerce').fillna(0.00).round(2).astype('float64')
