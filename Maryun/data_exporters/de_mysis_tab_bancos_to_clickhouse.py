...
TABLE = 'dwh.mysis_tab_bancos'
STAGING = 'dwh.mysis_tab_bancos_stg'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

# Full load atomico. El insert directo no propagaba las bajas del origen:
# un banco dado de baja en MySis quedaria vivo en ClickHouse para siempre.
...
INSERT_COLS = [
    'banco_id', 'codigoSBIF', 'banco_nombre', 'oa', 'propio'
]

INT_COLS_NOTNULL     = {'banco_id', 'oa', 'propio'}
INT_COLS_NULLABLE    = set()
DECIMAL_COLS_NOTNULL = set()
DECIMAL_COLS_NULLABLE= set()
FLOAT_COLS_NOTNULL   = set()
STRING_COLS_NOTNULL  = set()
STRING_COLS_NULLABLE = {'banco_nombre', 'codigoSBIF'}
...
def _prepare_for_insert(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in INSERT_COLS:
        if c not in df.columns:
            df[c] = pd.NA
    df = df[INSERT_COLS]
    df = df.dropna(subset=['banco_id'])
    _int_nn_defaults = {'oa': 1, 'propio': 0}
