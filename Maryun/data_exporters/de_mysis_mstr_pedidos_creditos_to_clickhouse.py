...
TABLE = 'dwh.mysis_mstr_pedidos_creditos'
STAGING = 'dwh.mysis_mstr_pedidos_creditos_stg'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

# Full load atomico. El insert incremental no propagaba las bajas del origen:
# una fila anulada en MySis quedaba viva en ClickHouse para siempre. Con
# staging + EXCHANGE TABLES la tabla queda identica al origen en cada corrida
# (altas, ediciones y bajas), sin ventana en que quede vacia.
MIN_RATIO = 0.5  # aborta si el origen trae menos de la mitad de lo que ya hay

INSERT_COLS = [
    'id', 'pago_id', 'importe', 'id_in', 'descripcion', 'usr_id', 'dt_in',
    'chebanco', 'chefecha', 'chenumero', 'caja_id', 'cartola_id', 'concilia_id',
    'deposito_banco', 'deposito_fecha', 'deposito_usr'
]

INT_COLS_NOTNULL     = {'id_in'}
INT_COLS_NULLABLE    = {'caja_id', 'cartola_id', 'concilia_id', 'deposito_banco', 'deposito_usr', 'id', 'pago_id', 'usr_id'}
DECIMAL_COLS_NOTNULL = set()
DECIMAL_COLS_NULLABLE= {'importe'}
FLOAT_COLS_NOTNULL   = set()
STRING_COLS_NOTNULL  = set()
STRING_COLS_NULLABLE = {'chebanco', 'chefecha', 'chenumero', 'descripcion'}

DATETIME_COLS = ['dt_in']
DATE_COLS     = ['deposito_fecha']
