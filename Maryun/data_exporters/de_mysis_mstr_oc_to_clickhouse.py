...
TABLE = 'dwh.mysis_mstr_oc'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

INSERT_COLS = [
    'oc_id', 'proveedor_id', 'dt_in', 'id_externo', 'dt_registro', 'dt_llega',
    'dt_cierre', 'usr_in', 'observacion', 'usr_solicita', 'usr_valida', 'dt_valida',
    'fpago', 'oc_fin', 'oc_fin_usr', 'oa', 'neto', 'iva', 'total', 'destino_id',
    'archivo', 'dt_lectura', 'dt_envio', 'rso_importacion', 'fecha_original',
    'importacion', 'versionoc'
]

INT_COLS_NOTNULL  = {'oa', 'importacion', 'versionoc'}
INT_COLS_NULLABLE = {'proveedor_id', 'usr_in', 'usr_solicita', 'usr_valida', 'oc_fin_usr'}
DECIMAL_COLS_NULLABLE = {'neto', 'iva', 'total'}
DECIMAL_COLS = DECIMAL_COLS_NULLABLE
STRING_COLS_NULLABLE = {
    'id_externo', 'observacion', 'fpago', 'destino_id',
    'archivo', 'rso_importacion'
}
DATE_COLS_DT64 = ['dt_in', 'dt_registro', 'dt_llega', 'dt_cierre', 'dt_valida', 'oc_fin']
DATE_COLS_DT   = ['dt_lectura', 'dt_envio']
DATE_COLS_D    = ['fecha_original']
ALL_DATE_COLS = DATE_COLS_DT64 + DATE_COLS_DT + DATE_COLS_D
...
def _prepare_for_insert(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in INSERT_COLS:
        if c not in df.columns:
            df[c] = pd.NA
    df = df[INSERT_COLS]
    df = df.dropna(subset=['oc_id'])
    defaults = {'oa': 1, 'importacion': 0, 'versionoc': 0}
...
def _coerce_dates_for_clickhouse(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    for c in DATE_COLS_DT64 + DATE_COLS_DT:
        if c in df.columns:
...
def export_data_to_clickhouse(df: pd.DataFrame, *args, **kwargs):
    """Insercion directa con ReplacingMergeTree. ClickHouse deduplica por oc_id."""
    client = _client()
