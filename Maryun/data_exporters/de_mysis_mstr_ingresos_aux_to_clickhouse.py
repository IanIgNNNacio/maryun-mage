...
# (system.columns): pu = Nullable(Decimal(18, 2)).
DECIMAL_SCALES = {'pu': decimal.Decimal('0.01')}
...
def _prepare_for_insert(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in DECIMAL_COLS.intersection(df.columns):
        # Se conserva el decimal.Decimal exacto que entrega MySQL: pasar por
        # float64 aqui ya introduce el error que luego trunca el centavo.
