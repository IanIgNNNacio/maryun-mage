...
TABLE = 'dwh.mysis_mstr_caja'
STAGING = 'dwh.mysis_mstr_caja_stg'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

# Full load atomico. El insert directo no propagaba las bajas del origen:
# una caja eliminada en MySis quedaria viva en ClickHouse para siempre.
# Con staging + EXCHANGE TABLES la tabla queda identica al origen en cada
# corrida (altas, ediciones y bajas), sin ventana en que quede vacia.
MIN_RATIO = 0.5  # aborta si el origen trae menos de la mitad de lo que ya hay

INSERT_COLS = [
    'caja_id', 'usr_id', 'dt_inicio', 'dt_fin', 'comentarios', 'dt_valida', 'usr_valida'
]

INT_COLS_NOTNULL     = {'caja_id'}
INT_COLS_NULLABLE    = {'usr_id', 'usr_valida'}
DECIMAL_COLS_NOTNULL = set()
DECIMAL_COLS_NULLABLE= set()
FLOAT_COLS_NOTNULL   = set()
STRING_COLS_NOTNULL  = set()
STRING_COLS_NULLABLE = {'comentarios'}

DATETIME_COLS = ['dt_inicio', 'dt_fin', 'dt_valida']
DATE_COLS     = []
