...
TABLE = 'dwh.mysis_tab_tipos'
CONFIG_PATH = '/home/src/Maryun/io_config.yaml'
PROFILE = 'maryun'

INSERT_COLS = [
    'tipo_id', 'tipo_descripcion', 'oa', 'user_in', 'dt_in', 'area'
]

INT_COLS_NOTNULL     = {'tipo_id'}
...
STRING_COLS_NULLABLE = {'area', 'tipo_descripcion'}
...
def _fetch_existing_ids(client, ids: list) -> Set[int]:
    ...
    for i in range(0, len(ids), step):
        ...
        q = f"SELECT tipo_id FROM dwh.mysis_tab_tipos WHERE tipo_id IN ({ids_txt})"
        res = client.query(q)
...
def export_data_to_clickhouse(df: pd.DataFrame, *args, **kwargs):
    ...
    for i in range(0, total, chunk_size):
        ...
        # Anti-join por tipo_id (PK)
        ids = chunk['tipo_id'].dropna().astype(int).tolist()
        existing = _fetch_existing_ids(client, ids)

        if existing:
            keep_mask = [int(x) not in existing for x in chunk['tipo_id']]
