...
@data_loader
def load_data(*args, **kwargs) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Outputs (en este orden):
      1) rcvsii_df
      2) billingInstallmentPlans_df
    """
    cfg = ConfigFileLoader(CONFIG_PATH, PROFILE)
    ...
    return rcvsii_df, billingInstallmentPlans_df
...
def test_output(output, *args) -> None:
    ...
    """
    outputs = (output,) + args
    assert len(outputs) >= 2, "Se esperaban 2 outputs: rcvsii, costCenter"
