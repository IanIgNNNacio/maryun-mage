...
WHERE
  ...
  AND o.dt_out >= DATE_SUB(NOW(), INTERVAL {{dias}} DAY)
  AND o.dt_out < DATE_ADD(CURDATE(), INTERVAL 3 DAY)
  -- AND o.dt_out >= DATE_SUB(NOW(), INTERVAL 5000 MINUTE)
  -- AND o.dt_out <= NOW()
...
WHERE
  o1.direccion_id <> 0
  and o1.dt_out >= DATE_SUB(NOW(), INTERVAL {{dias}} DAY)
  AND o1.dt_out < DATE_ADD(CURDATE(), INTERVAL 3 DAY)
  -- AND o1.dt_out >= DATE_SUB(NOW(), INTERVAL 5000 MINUTE)
  -- AND o1.dt_out <= NOW()
...
WHERE
  a.desde < DATE_ADD(CURDATE(), INTERVAL 3 DAY)
  AND a.desde >= DATE_SUB(NOW(), INTERVAL {{dias}} DAY)
  -- a.desde >= DATE_SUB(NOW(), INTERVAL 5000 MINUTE)
  -- AND a.desde <= NOW()
  -- a.desde >= '{{ start_date }}'